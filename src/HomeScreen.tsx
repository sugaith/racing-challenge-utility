import {ActivityIndicator, Text, FlatList, Pressable, View} from 'react-native'
  ...
  return (
    <Pressable>
      <View>
        ...
        <Text><Text>weight: </Text>{weight}</Text>
        <Text><Text>color: </Text>{color}</Text>
      </View>
...
const HomeScreen = () => {
  ...
  const [queryRacers, { data, loading, error }] = useLazyQuery<RacersResponse>(QUERY_RACERS, {
  ...
  if (error) {
    return <Text>{`Error when fetching racers: ${JSON.stringify(error)}`}</Text>
  }
  ...
  return (
    <View>
      <Pressable onPress={() => queryRacers()}><Text> Get new Racers </Text></Pressable>
...

      {
        ...
        <Pressable onPress={startRacing}><Text>Start Racing!</Text></Pressable>
      )}
